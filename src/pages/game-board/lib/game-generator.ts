...
class WaterSortPuzzleGenerator {
  ...
  private validateConfig(config: GeneratorConfig): void {
    if (config.numColors < GAME_CONFIG.MIN_COLORS) {
      throw new Error(
        `색상 개수는 최소 ${GAME_CONFIG.MIN_COLORS}개 이상이어야 합니다.`,
      );
    }
    if (config.bottleHeight < GAME_CONFIG.MIN_BOTTLE_HEIGHT) {
      throw new Error(
        `병의 높이는 최소 ${GAME_CONFIG.MIN_BOTTLE_HEIGHT} 이상이어야 합니다.`,
      );
    }
  }
  ...
  generate(): Puzzle {
    ...
    while (true) {
      ...
      if (
        this.isCompletelyMixed(mixedState) &&
        this.hasValidStructure(mixedState)
      ) {
    ...
    throw new Error(
      `${GAME_CONFIG.MAX_GENERATION_ATTEMPTS}번 시도 후에도 유효한 퍼즐을 생성할 수 없습니다.`,
    );
  }
  ...
  private createDirectlyMixedState(): Puzzle {
    ...
    const filledBottles =
      this.config.numBottles - GAME_CONFIG.MIN_EMPTY_BOTTLES;
    ...
    for (let bottleIdx = 0; bottleIdx < filledBottles; bottleIdx++) {
      ...
      for (
        let i = 0;
        i < this.config.bottleHeight && colorIndex < allColors.length;
        i++
      ) {
  ...
  private hasValidStructure(state: Puzzle): boolean {
    ...
    if (colorCounts.size !== this.config.numColors) {
      console.log(
        `색상 개수 불일치: 기대 ${this.config.numColors}, 실제 ${colorCounts.size}`,
      );
      return false;
    }
    ...
    for (const [color, count] of colorCounts.entries()) {
      if (count !== this.config.bottleHeight) {
        console.log(
          `색상 ${color}의 개수 불일치: 기대 ${this.config.bottleHeight}, 실제 ${count}`,
        );
        return false;
      }
    }
    ...
    if (emptyBottles < GAME_CONFIG.MIN_EMPTY_BOTTLES) {
      console.log(
        `빈 병 부족: 최소 ${GAME_CONFIG.MIN_EMPTY_BOTTLES}개 필요, 실제 ${emptyBottles}개`,
      );
      return false;
    }
    ...
    for (let i = 0; i < state.length; i++) {
      ...
      if (bottle.length === 0) {
        ...
        if (uniqueColors.size === 1) {
          ...
          mixedBottles++;
          console.log(
            `병 ${i}: 섞인 병 (색상: ${Array.from(uniqueColors).join(', ')})`,
          );
        }
      }
    }
