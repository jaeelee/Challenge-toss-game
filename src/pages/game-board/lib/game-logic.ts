...
export class LiquidMover {
  ...
  private calculateGameScore(state: Puzzle): number {
    ...
    for (const bottle of state) {
      ...
      score += colorGroups.reduce(
        (sum, group) => sum + group.length * group.length,
        0,
      );
   ...
   * 병 내의 색상 그룹 분석
   */
  private getColorGroups(
    bottle: Bottle,
  ): Array<{ color: Color; length: number }> {
...
export class GameUtils {
  ...
  static printMoveResult(result: MoveResult, move: Move): void {
    if (result.success) {
      console.log(
        `✅ 이동 성공: 병 ${move.from} → 병 ${move.to} (${result.actualAmount}개)`,
      );
    } else {
  ...
  static printGameStatus(state: Puzzle, gameAPI: GameAPI): void {
    ...
    possibleMoves.forEach((move, index) => {
      console.log(
        `${index + 1}. 병 ${move.from} → 병 ${move.to} (${move.amount}개)`,
      );
    });
...
export function testLiquidMovement() {
  ...
  possibleFromBottle2.forEach((move, index) => {
    const score = gameAPI.evaluateMove(
      currentState,
      move.from,
      move.to,
      move.amount,
    ...
    console.log(
      `   ${index + 1}. 병 ${move.to}로 ${move.amount}개 이동 (점수: ${score})`,
    );
  });
}
