...
5. **Invariant risk in game logic** (cross-check with what `game-logic` agent would care about): does the diff touch `LiquidMover.executeMove`, `calculateActualMoveAmount`, or the generator's color-distribution logic in a way that could break color-count conservation or bottle-capacity limits? If so, flag it even if tests pass, since coverage may be thin.
6. **Git workflow conventions** (see `docs/conventions/git.md`). Commit message format is already enforced at commit time by the `commitlint` + `husky` `commit-msg` hook, so you don't need to re-validate well-formed messages — but do flag: a PR/branch targeting `main` directly for non-`hotfix/*` work (should target `develop`), or a branch name that doesn't follow `<type>/<slug>` (e.g. missing the `feat`/`fix`/`chore`/`hotfix` prefix).
